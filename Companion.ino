...
long lastTime = 0;
long lastMSunPV = 0;
String Months[13] = { "Mois", "Jan", "Fev", "Mars", "Avril", "Mai", "Juin", "Juil", "Aout", "Sept", "Oct", "Nov", "Dec" };
...
int dim = 200;  // Eclairage intermédiaire au lancement
bool inverse = true;
int x;
...
void Affiche() {
  ...
  if (icone == "01d") {
    meteo.pushImage(0, 0, 50, 50, clear_day);
    goto suite;
  }
  if (icone == "01n") {
    meteo.pushImage(0, 0, 50, 50, clear_night);
    goto suite;
  }
  if (icone == "02d") {
    meteo.pushImage(0, 0, 50, 50, partly_cloudy_day);
    goto suite;
  }
  if (icone == "02n") {
    meteo.pushImage(0, 0, 50, 50, partly_cloudy_night);
    goto suite;
  }
  if ((icone == "03d") or (icone == "03n")) {
    meteo.pushImage(0, 0, 50, 50, few_clouds);
    goto suite;
  }
  if ((icone == "04d") or (icone == "04n")) {
    meteo.pushImage(0, 0, 50, 50, cloudy);
    goto suite;
  }
  if ((icone == "09d") or (icone == "09n")) {
    meteo.pushImage(0, 0, 50, 50, rain);
    goto suite;
  }
  if ((icone == "10d") or (icone == "10n")) {
    meteo.pushImage(0, 0, 50, 50, lightRain);
    goto suite;
  }
  if ((icone == "11d") or (icone == "11n")) {
    meteo.pushImage(0, 0, 50, 50, thunderstorm);
    goto suite;
  }
  if ((icone == "13d") or (icone == "13n")) {
    meteo.pushImage(0, 0, 50, 50, snow);
    goto suite;
  }
  if ((icone == "50d") or (icone == "50n")) {
    meteo.pushImage(0, 0, 50, 50, fog);
    goto suite;
  }
  if (icone == "80d") {
    meteo.pushImage(0, 0, 50, 50, splash);
    goto suite;
  }
  if (ID == "301") {
    meteo.pushImage(0, 0, 50, 50, drizzle);
    goto suite;
  }
...
void AfficheCumul() {
  ...
  sprite.fillSprite(TFT_BLACK);
  sprite.setTextColor(TFT_WHITE, TFT_BLACK);
  sprite.drawString("CUMULS en kWh", 160, 10, 2);
...
void decrypte() {
  ...
  ***************************************************************************************/
  ...
  CUMBAL = MsgSplit2[4];  // Cumul Ballon cumulus
  //*************************************************************************************
...
void indic() {
  ...
  if (valeur > residuel) {          // Permet l'affichage d'au moins une barre si on produit
  ...
  if (valeur != 0) {
    if (valeur < 0) sprite.fillRect(200, 153, 20, 9, TFT_WHITE);
    else sprite.fillRect(200, 158, 20, 4, color1);
...
void donneesmeteo() {
  ...
  Serial.println(strTime(forecast->dt[0]));
  ...
  Serial.println(forecast->temp[0]);
  Serial.print("description         : ");
  Serial.println(forecast->description[0]);
  Serial.print("icone               : ");
  Serial.println(forecast->icon[0]);
  Serial.print("ID                  : ");
  Serial.println(forecast->id[0]);
...
void getArrivals() {
  ...
  while (!client.available()) {
    Serial.println("Attente 5ms de la réponse serveur...");
    delay(5);
  }
  ...
  Serial.println("Requete validée par le serveur");
